...
for i, s in enumerate(subGraphs):
    ...
    for j in range(len(list_xy_coords_nodes) - 1):
        ...
        if not s.has_edge(node1, node2) and not s.had_edge(node2, node1):
            print("theres a problem! - don't accept the track - test2")
...
for i, s in enumerate(subGraphs):
    ...
    for j in range(len(list_xy_coords_nodes) - 1):
        ...
        node2 = list_xy_coords_nodes[j+1][0]
        if not s.has_edge(node1, node2) and not s.had_edge(node2, node1):
            print("theres a problem! - don't accept the track - test3")
...
for i, s in enumerate(subGraphs):
    ...
    for j in range(len(sorted_vivl_ids) - 1):
        ...
        if not s.has_edge(node1, node2) and not s.had_edge(node2, node1):
            print("theres a problem! - don't accept the track - test4")
