...
# used when clustering on updated states - resetting all edge components - brute force
...
def cluster(subGraph):
    ...
    for node in subGraph.nodes(data=True):
        ...
        if smallest_dist < KL_thres:
            ...
            # if smallest distance is less than KL threshold, then merge & recalculate distances to the merged state
            ...
            subGraph.nodes[node_num]["merged_cov"] = merged_cov
...
if __name__ == "__main__":
    ...
    global KL_lut
    global mapping
    global TRACK_STATE_KEY
    inputDir = args.input
    outputDir = args.output
    TRACK_STATE_KEY = args.dict
    KL_lut = args.lut
    mapping = load_lut(KL_lut)      # load LUT: empirical variance: {upper bound emp var bin: KL_dist upper bound threshold}
    ...
    # parallelizing the clustering
    pool = Pool(os.cpu_count())
    pool.imap(cluster, subGraphs)
    pool.close()
    pool.join()

    # reweight the mixture based on inward active edges & compute priors for a node based on inward edges
    h.compute_mixture_weights(subGraphs)
